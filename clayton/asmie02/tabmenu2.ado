...
*! version 3.1.0 (added graph option)
...
global ta_lev `level'
global ta_gra 1
if "`graph'"=="" {global ta_gra 0}
global ta_log 1
if "`log'"=="" {global ta_log 0}
...
if "$ta_typ"=="failure" | "$ta_typ"=="count" {
	...
	sort $ta_mod $ta_exp
	if $ta_gra==1 & $ta_log==0 {
	graph table2 $ta_exp, c(l) xlab ylab l1("response")  by($ta_mod)
	}
	if $ta_gra==1 & $ta_log==1 {
	graph table2 $ta_exp, c(l) xlab ylab l1("response")  ylog  by($ta_mod)
	}
...
if "$ta_typ"=="metric" {
	...
	if $ta_gra==1 & $ta_log==1 {
	graph table2 $ta_exp, sort c(l) xlab ylab l1("response")  ylog by($ta_mod)
	}
        if $ta_fr==1 { local contents "table1 `contents'"}
        if $ta_ci==1 { local contents "`contents' `ci'" }
        label var table2 $ta_res
