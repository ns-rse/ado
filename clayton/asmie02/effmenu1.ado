...
qui inspect $ef_exp
if r(N_unique)>20 & "`$ef_exp[type]'"!="metric" {
    display as error" More than 20 values for exposure ($ef_exp)" 
    display as text "Should it be declared as metric?"
    exit
}
if "$ef_mod" != "" {
    qui inspect $ef_mod
    if r(N_unique)>20 & "`$ef_mod[type]'"!="metric" {
        display as error " More than 20 values for modifier ($ef_mod)"
...
if "`$ef_exp[type]'" == "categorical" {
    ...
    if "$ef_mod" == "" {
        window control check "Previous level contrasts" 200 130 80 10 ef_prev
    }
    else { global ef_prev 0 }
}
...
if "$ef_mod" != "" & $ef_prev==1 {
    di in re "Previous level contrasts not allowed with a modifier"
    exit
}
if _rc==3000 {
  exit
...
if _rc==3001 {
	...
	if $ef_prev==1 {local effmenu2 "`effmenu2' prev"}
        di
        di as res "`effmenu2'"
        di
    }
